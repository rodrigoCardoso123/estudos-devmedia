const questoes = [
  ...
  function embaralharArray(questoes, quantidade) { // Função que retorna 10 perguntas aleatorias
    const perguntasEmbaralhadas = questoes.sort(() => Math.random() - 0.5); // Embaralha as perguntas do array questoes
    return perguntasEmbaralhadas.slice(0, quantidade);  // retorna um novo array com 10 perguntas aleatórias selecionadas
  }
  
  function exibirPerguntas(index, pergunta) { // Função que exibe a pergunta e retornar a resposta do usuário
      console.log(`Pergunta ${index + 1}: ${pergunta.pergunta}`);
      let respostaUsuario = entradaDados.questionFloat('Digite a resposta (ano): ');
      return respostaUsuario; 
  }

  function validarResposta(respostaDoUsuario, pergunta){ // Função que valida a resposta do usuário
    return respostaDoUsuario == pergunta.resposta;
  }

  function exibirResultado(nome,pontuacao){
    let mensagem = ``
  ...
  const perguntasSelecionadas = embaralharArray(questoes, quantidade);

  perguntasSelecionadas.forEach((pergunta, index) => {
    const respostaUsuario = exibirPerguntas(index, pergunta);
    const validarR = validarResposta(respostaUsuario, pergunta);
  ...
  exibirResultado(nome,pontuacao);
